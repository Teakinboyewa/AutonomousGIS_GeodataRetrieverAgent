...
class AGGRADockWidget(QtWidgets.QDockWidget, FORM_CLASS):
    ...
    def setup_initial_rows(self, keys_directory=keys_dir):
        ...
        # Only show datasources that have corresponding .keys files (i.e., need API keys)
        datasources_with_keys = [os.path.splitext(f)[0] for f in keys_files]
        
        # Add a row for each datasource that requires an API key (has a .keys file)
        for datasource in sorted(datasources_with_keys):
    ...
    def add_row(self, key_name = None, keys_directory=keys_dir):
        ...
        # Get the list of datasource names that have API keys (from .keys files in the 'Keys' directory)
        # Only include datasources that need API keys (have corresponding .keys files)
        if keys_directory:
            keys_files = [f for f in os.listdir(keys_directory) if f.endswith('.keys') and f != 'template.keys']
            datasources_with_keys = [os.path.splitext(f)[0] for f in keys_files]

            # Add the datasource name options (only those that require API keys)
            combo_box.addItems(sorted(datasources_with_keys))
